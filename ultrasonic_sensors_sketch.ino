...
void loop() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (distanceCm >= 1 && distanceCm <= 30) { strcat(durationString, "ROUGE"); }  // Danger zone, dangerously close to obstacle, should stop
    ...
    else if (distanceCm > 70 && distanceCm <= 100) { strcat(durationString, "JAUNE"); }  // Robot getting closer to an obstacle and should consider changing directions
    else {
      strcat(durationString, "VERT");
    ...
    if (i == 0) {
      ...
      strcat(direction, "Angle");
    } else {
      strcat(direction, "Side");
